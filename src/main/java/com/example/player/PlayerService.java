...
public class PlayerService implements PlayerRepository {
    ...
    public Player addPlayer(Player player) {
        count += 1;
        Player newPlayer = new Player(count, player.getPlayerName(), player.getJerseyNumber(), player.getRole());
        team.put(count, newPlayer);
        return newPlayer;
    }
    ...
    public Player updatePlayer(int playerId, Player player) {
        ...
        if (player.getPlayerName() != null) {
            existingPlayer.setPalyerName(player.getPlayerName());
        }
